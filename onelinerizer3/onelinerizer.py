...
def get_init_code(tree, table):
    ...
    output = provide(
        ...
        __print=T("__import__({!r}, level=0).__dict__['print']").format(BUILTINS_MODULE),
        __y="(lambda f: (lambda x: x(x))(lambda y:"
          " f(lambda: y(y)())))",
        __g=T("globals()"),
        __contextlib=T("((lambda __m: (__m if hasattr(__m, 'nested') else (setattr(__m, 'nested', (lambda *cms: (lambda __s: ([__s.enter_context(c) for c in cms], __s)[1])(__m.ExitStack())))) or __m)))(__import__('contextlib', level=0))"),
...
class Namespace(ast.NodeVisitor):
    ...
    def visit_Call(self, tree):
        ...
        keywords = [self.visit(kw) for kw in getattr(tree, 'keywords', [])]
        starargs = []
        if getattr(tree, 'starargs', None) is not None:
            starargs = ["*" + self.visit(tree.starargs)]
        kwargs = []
        if getattr(tree, 'kwargs', None) is not None:
    ...
    def visit_Try(self, tree):
        # Python 3 uses ast.Try for try/except and try/finally nodes.
        if tree.handlers:
            if tree.finalbody:
                ...
                for handler in tree.handlers:
                    ...
                    if handler.name is not None:
                        name_code = (self.store_var(handler.name)
                                     if isinstance(handler.name, str)
                                     else self.visit(handler.name))
                        code = code.format(body=assignment_component(
                            T('{body}'), name_code, '__value'))
                ...
                return \
                    lambda_function({'__out': '[None]'}).format(
                        lambda_function({
                            '__ctx': T(
                                "{__contextlib}.nested(type('finally', (), {{"
            ...
            return self.visit_TryExcept(tree)
        return self.visit_TryFinally(tree)
    ...
    def visit_arguments(self, tree):
        ...
        arg_names = []
        args = []
        for d, a in zip(padded_defaults, tree.args):
            name = a.arg if hasattr(a, 'arg') else a.id
            arg_names.append(name)
            args.append(name if d is None else name + "=" + self.visit(d))
        if getattr(tree, 'vararg', None) is not None:
            vararg = tree.vararg.arg if hasattr(tree.vararg, 'arg') else tree.vararg
            args += ["*" + vararg]
            arg_names += [vararg]
        if getattr(tree, 'kwarg', None) is not None:
            kwarg = tree.kwarg.arg if hasattr(tree.kwarg, 'arg') else tree.kwarg
            args += ["**" + kwarg]
            arg_names += [kwarg]
        if getattr(tree, 'kwonlyargs', None):
            for arg, default in zip(tree.kwonlyargs, tree.kw_defaults):
                name = arg.arg if hasattr(arg, 'arg') else arg.id
                arg_names.append(name)
                args.append(name if default is None else name + "=" + self.visit(default))
        args = T(', ').join(args)
    ...
    def visit_FormattedValue(self, tree):
        value = self.visit(tree.value)
        if getattr(tree, 'conversion', -1) != -1:
            if tree.conversion == ord('s'):
                value = T('str({})').format(value)
            elif tree.conversion == ord('r'):
                value = T('repr({})').format(value)
            elif tree.conversion == ord('a'):
                value = T('ascii({})').format(value)
        fmt = '' if tree.format_spec is None else ', ' + self.visit(tree.format_spec)
        return T('format({}{})').format(value, fmt)

    def visit_JoinedStr(self, tree):
        return T(' + ').join(self.visit(value) for value in tree.values)
    ...
    def visit_NameConstant(self, tree):
        return T('{!r}').format(tree.value)

    def visit_Constant(self, tree):
        value = tree.value
        if value is None:
            return T('None')
        return T('{!r}').format(value)
    ...
    def visit_Raise(self, tree):
        exc = getattr(tree, 'exc', None) if hasattr(tree, 'exc') else tree.type
        if exc is None:
            return T('([] for [] in []).throw(*{__sys}.exc_info())')
        if getattr(tree, 'cause', None) is not None:
            return T('([] for [] in []).throw({})').format(self.visit(exc))
        if hasattr(tree, 'inst'):
        ...
        for handler in tree.handlers:
            if handler.type is None:
                ...
                code = T('issubclass(__exctype, {type}) and {body}').format(
                    type=self.visit(handler.type))
            if handler.name is not None:
                name_code = (self.visit(handler.name)
                             if not isinstance(handler.name, str)
                             else self.store_var(handler.name))
                code = code.format(body=assignment_component(
                    T('{body}'), name_code, '__value'))
            handlers.append(code.format(
                body=assignment_component(
    ...
    def visit_With(self, tree):
        ...
        for i, item in enumerate(reversed(tree.items)):
            ...
            code = T('({}, {cm}.__exit__(None, None, None))[0]').format(code, cm=cm_name)
...
def onelinerize(original):
    ...
    simple_stmt_types = [ast.Delete, ast.Assign, ast.AugAssign,
                         ast.Raise, ast.Assert, ast.Import, ast.ImportFrom,
                         ast.Global, ast.Expr, ast.Pass]
    if hasattr(ast, 'Print'):
        simple_stmt_types.append(ast.Print)
    if hasattr(ast, 'Exec'):
        simple_stmt_types.append(ast.Exec)
    if len(original.splitlines()) == 1 and \
       len(t.body) == 1 and \
       type(t.body[0]) in tuple(simple_stmt_types):
        return original
